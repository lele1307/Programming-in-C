...
int main(int argc,char **argv){
    ...
    {
        test(); 
        printf("Your input number list is '%s'.\n", argv[1]);
    ...
    {
        printf("ERROR: Incorrect usage, try e.g. %s 513276048\n", argv[0]);
    }
    return 0;
}
void test()
    ...
    Neill_SDL_ReadFont(fontdata,"mode7.fnt");
        for(i=arrLen-1; i>=0; i--)
        {
            ...
            printf("\n");
                for ( m = 0; m < 9; m++)
                {
                    Neill_SDL_DrawChar(&sw, fontdata, boards[array[i]].b[m], m%3*RECTSIZE, m/3*RECTSIZE);
                }
